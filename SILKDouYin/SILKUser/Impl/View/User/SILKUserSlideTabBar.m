...
@implementation SILKUserSlideTabBar
...
-(void)layoutSubviews {
    ...
    CGFloat itemWidth = _itemWidth = SCREEN_WIDTH/_titles.count;
    [_titles enumerateObjectsUsingBlock:^(NSString * title, NSUInteger idx, BOOL *stop) {
        ...
        if(idx != self.titles.count - 1) {
            ...
            spliteLine.backgroundColor = [UIColor colorWithWhite:1 alpha:0.2];
            spliteLine.layer.zPosition = 10;
            [self addSubview:spliteLine];
        }
    }];
    _labels[_tabIndex].textColor = [UIColor colorWithWhite:1 alpha:1];
    
    _slideLightView = [[UIView alloc] init];
    _slideLightView.backgroundColor = [UIColor colorWithRed:250/255.f green:206/255.f blue:21/255.f alpha:1];
              ...
              initialSpringVelocity:0
                         ...
                         animations:^{
                             ...
                             [self.labels enumerateObjectsUsingBlock:^(UILabel *label, NSUInteger idx, BOOL *stop) {
                                 label.textColor = index == idx ? [UIColor colorWithWhite:1 alpha:1] : [UIColor colorWithWhite:1 alpha:0.6];
                             }];
                         } completion:^(BOOL finished) {
                             [self.delegate tabBarTapAction:index];
                         }];
